...
public class AnkiCollection
{
    ...
    public long[] CreateNote(long deckId, long noteTypeId, params string[] fields)
    {
        ...
        return new long[] { newNoteId, newCardId };
    }
    ...
    /// <summary>
    /// Creates and adds a new revision log entry to the collection.
    /// </summary>
    /// <param name="cardId">ID of the card being reviewed</param>
    /// <param name="ease">Value of the answer (1-4 for Review, 1-3 for Learn/Relearn)</param>
    /// <param name="interval">Current interval in days</param>
    /// <param name="lastInterval">Previous interval in days</param>
    /// <param name="factor">Learning factor (interval multiplier)</param>
    /// <param name="timeTookMs">Time spent answering in milliseconds</param>
    /// <param name="revisionType">Type of revision (Review, Learn, Relearn)</param>
    /// <returns>The ID (timestamp) of the created revision log</returns>
    /// <exception cref="ArgumentException">Thrown when cardId doesn't exist or ease value is invalid for the revision type</exception>
    public long CreateRevisionLog(
        long cardId,
        long ease,
        long interval,
        long lastInterval,
        ...
        long createdAtMs)
    {
        ...
        {
            throw new ArgumentException($"Unknown card id '{cardId}' in this collection");
        }

        // Validate ease value based on revision type
        bool isValidEase = revisionType switch
        {
            RevisionType.Review => ease >= 1 && ease <= 4,
            RevisionType.Learn or RevisionType.Relearn => ease >= 1 && ease <= 3,
            _ => false
        };

        if (!isValidEase)
        {
            throw new ArgumentException($"Invalid ease value '{ease}' for revision type {revisionType}");
        }

        var id = createdAtMs == 0 ? DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() : createdAtMs;
        while (_revLogs.ContainsKey(id))
        {
            id++;
        }

        var usn = 0L; // Update sequence number starts at 0 for new logs
        var revLog = new AnkiRevLog(id, cardId, usn, ease, interval, lastInterval, factor, timeTookMs, revisionType);
        _revLogs.Add(id, revLog);
